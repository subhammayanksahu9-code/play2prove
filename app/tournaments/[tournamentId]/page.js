...
function normalizeStatus(rawStatus, tournament, results = []) {
  ...
  if (raw === "live" || raw === "match ongoing" || raw === "ongoing") return "LIVE";
  ...
  if (raw === "completed" || raw === "past") {
    const hasPendingResults = results.some((item) => safeText(item?.result_status, "pending").toLowerCase() === "pending");
    return hasPendingResults ? "CALCULATION ONGOING" : "COMPLETED";
  }
  const now = Date.now();
  const start = parseIST(tournament?.matchDate, tournament?.startTime);
  const open = tournament?.registrationOpensAt ? new Date(tournament.registrationOpensAt).getTime() : null;
  const close = tournament?.registrationClosesAt ? new Date(tournament.registrationClosesAt).getTime() : null;
...
function Countdown({ target, label = "STARTS IN" }) {
  ...
  if (!target) return <div className="tdCountdown muted"><span>{label}</span><strong>—</strong></div>;
  if (!remaining) return <div className="tdCountdown live"><span>{label}</span><strong>NOW</strong></div>;
  return <div className="tdCountdown"><span>{label}</span><strong>{days > 0 ? `${days}d ` : ""}{String(hours).padStart(2, "0")}:{String(minutes).padStart(2, "0")}:{String(seconds).padStart(2, "0")}</strong></div>;
}
...
export default function TournamentDetailPage() {
  ...
  const registrationOpen = ["UPCOMING", "REGISTRATION OPEN"].includes(status) && remainingSlots > 0;
  ...
  const notice = status === "CALCULATION ONGOING" ? "Results are being verified. New registrations are disabled." : status === "LIVE" ? "Only registered participants should use the released match information." : "Entry fee and eligibility are validated on the server before registration is confirmed.";
  ...
  return (
    <main className="tdPage">
        ...
        <div className="tdLayout">
          <div className="tdMain">
            <section className="tdHero">
              ...
              <div className="tdHeroContent">
                ...
                <div className="tdHeroFooter">
                  ...
                  <div className="tdHeroSignal"><span className="tdSignalDot" />{status === "CALCULATION ONGOING" ? "CALCULATING RESULTS" : status === "LIVE" ? "MATCH IN PROGRESS" : "TOURNAMENT STATUS"}</div>
                </div>
              </div>
            </section>
            ...
            {activeTab === "overview" && <section className="tdSectionGrid">
              <div className="tdPanel tdOverviewPanel"><div className="tdPanelHead"><div><span className="tdEyebrow">TOURNAMENT</span><h2>Overview</h2></div><span className={`tdMiniStatus ${statusClass}`}>{status}</span></div><div className="tdInfoGrid">
                {[["GAME", tournament.game.name], ["FORMAT", tournament.matchType.toUpperCase()], ["MAP", tournament.mapName], ["DATE", formatDate(tournament.matchDate)], ["START TIME", formatTime(tournament.startTime)], ["TIME SLOT", "—"], ["REGISTRATION", status === "REGISTRATION OPEN" ? "OPEN" : status], ["CAPACITY", capacity ? `${capacity} players` : "—"]].map(([label, value]) => <div className="tdInfoItem" key={label}><span>{label}</span><strong>{value}</strong></div>)}
              </div></div>
            ...
            {activeTab === "match" && <section className="tdSectionGrid tdMatchGrid"><div className="tdPanel"><div className="tdPanelHead"><div><span className="tdEyebrow">MATCH COMMAND</span><h2>Match information</h2></div></div><div className="tdInfoGrid">{[["GAME", tournament.game.name], ["MODE", tournament.matchType.toUpperCase()], ["MAP", tournament.mapName], ["DATE", formatDate(tournament.matchDate)], ["START", formatTime(tournament.startTime)], ["STATUS", status]].map(([label, value]) => <div className="tdInfoItem" key={label}><span>{label}</span><strong>{value}</strong></div>)}</div></div><div className="tdPanel tdMatchCountdown"><Countdown target={startTimestamp} label={status === "LIVE" ? "MATCH STATE" : "STARTS IN"}/><div className={`tdRoom ${roomUnlocked ? "released" : "locked"}`}><span className="tdEyebrow">ROOM ACCESS</span><strong>{roomUnlocked ? "ROOM RELEASED" : "ROOM LOCKED"}</strong>{roomUnlocked ? <div className="tdRoomCredentials"><div><span>ROOM ID</span><code>{room.id}</code><button onClick={() => navigator.clipboard?.writeText(room.id)} aria-label="Copy room ID">COPY</button></div><div><span>PASSWORD</span><code>{room.password}</code><button onClick={() => navigator.clipboard?.writeText(room.password)} aria-label="Copy room password">COPY</button></div></div> : <p>Room credentials appear here only after authorized release.</p>}</div></div></section>}
          </div>
          ...
          <aside className="tdRail">
            <div className="tdActionPanel">
              ...
              {registration ? <div className="tdRegistered"><div className="tdRegisteredMark">✓</div><span className="tdEyebrow">REGISTRATION CONFIRMED</span><h2>You're in.</h2><div className="tdRegistrationId"><span>REGISTRATION ID</span><strong>{registration.registrationId}</strong></div><div className="tdRegisteredMeta"><div><span>PLAYER</span><strong>{registration.gameUsername}</strong></div><div><span>GAME UID</span><strong>{registration.gameUid}</strong></div>{registration.teamName && <div><span>TEAM</span><strong>{registration.teamName}</strong></div>}</div><button className="tdPrimaryButton" onClick={() => setActiveTab("match")}>VIEW MATCH INFO <b>→</b></button></div> : <><button className="tdJoinCta" onClick={openJoin} disabled={!registrationOpen}>{status === "REGISTRATION CLOSED" ? "REGISTRATION CLOSED" : status === "FULL" || remainingSlots === 0 ? "TOURNAMENT FULL" : status === "CALCULATION ONGOING" ? "CALCULATION ONGOING" : status === "COMPLETED" ? "VIEW RESULTS" : "JOIN NOW"}<b>→</b></button><div className="tdRailStats"><div><span>ENTRY</span><strong>{formatMoney(tournament.entryFee)}</strong></div><div><span>PRIZE POOL</span><strong>{formatMoney(tournament.prizePool)}</strong></div><div><span>PER KILL</span><strong>{formatMoney(tournament.perKillReward)}</strong></div></div><div className="tdRailSlots"><div><span>SLOTS AVAILABLE</span><strong>{remainingSlots}</strong></div><div className="tdProgress"><span style={{ width: `${progress}%` }} /></div><small>{registeredCount} registered · {capacity || "—"} capacity</small></div><Countdown target={startTimestamp} label={status === "LIVE" ? "MATCH STATE" : "STARTS IN"}/><div className="tdRailRoom"><span className="tdEyebrow">ROOM STATUS</span><strong>{roomUnlocked ? "RELEASED" : "LOCKED"}</strong><small>{roomUnlocked ? "Credentials are available in Match Info." : "Only eligible registered players can view room credentials."}</small></div></>}
      ...
      {joinOpen && <div className="tdModalBackdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) resetJoin(); }}><div className="tdModal" role="dialog" aria-modal="true" aria-labelledby="td-registration-title"><button className="tdModalClose" onClick={resetJoin} aria-label="Close registration">×</button><div className="tdModalHead"><span className="tdEyebrow">SECURE REGISTRATION</span><h2 id="td-registration-title">{joinStep === 3 ? "Registration complete" : joinStep === 2 ? "Review registration" : "Join tournament"}</h2><div className="tdSteps"><span className={joinStep >= 1 ? "active" : ""}>01 DETAILS</span><span className={joinStep >= 2 ? "active" : ""}>02 REVIEW</span><span className={joinStep >= 3 ? "active" : ""}>03 CONFIRMED</span></div></div>{joinError && <div className="tdFormError">{joinError}</div>}
        ...
        {joinStep === 3 && <div className="tdSuccess"><div className="tdSuccessIcon">✓</div><span className="tdEyebrow">REGISTRATION CONFIRMED</span><h2>You are registered.</h2><p>Your registration has been recorded and the tournament state has been refreshed.</p><div className="tdSuccessCard"><div><span>REGISTRATION ID</span><strong>{joinSuccess?.registration_id || registration?.registrationId || "—"}</strong></div><div><span>PAYMENT</span><strong>{joinSuccess?.payment_status || "paid"}</strong></div><div><span>ROOM</span><strong>{roomUnlocked ? "RELEASED" : "LOCKED"}</strong></div></div><button className="tdPrimaryButton" onClick={resetJoin}>DONE</button></div>}</div></div>}
    </main>
  );
}
