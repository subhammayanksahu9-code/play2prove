...
export async function GET(request, { params }) {
  try {
    ...
    const [{ data: participants, error: participantsError }, { data: results, error: resultsError }] = await Promise.all([
      admin.from("tournament_participants").select("id,tournament_id,user_id,team_name,player_name,game_uid,joined_at,status").eq("tournament_id", tournament.id).not("status", "in", "(cancelled,disqualified)").order("joined_at", { ascending: true }),
      admin.from("tournament_results").select("id,tournament_id,user_id,rank,kills,points,prize_amount,result_status,created_at").eq("tournament_id", tournament.id).order("rank", { ascending: true, nullsFirst: false }),
    ]);
    if (participantsError) throw participantsError;
    if (resultsError) throw resultsError;

    const participantUserIds = [...new Set((participants || []).map((p) => asText(p.user_id)).filter(Boolean))];
    const resultUserIds = [...new Set((results || []).map((r) => asText(r.user_id)).filter(Boolean))];
    const profileIds = [...new Set([...participantUserIds, ...resultUserIds])];
    let profiles = [];
    if (profileIds.length) {
      const { data, error } = await admin.from("profiles").select("id,username,full_name,avatar_url").in("id", profileIds);
      if (error) throw error;
      profiles = data || [];
    }
    const profileMap = new Map(profiles.map((p) => [asText(p.id), p]));
    ...
    if (user) {
      ...
      if (ownEntry || ownParticipant) {
        ...
        if (asText(tournament.room_id) && asText(tournament.room_password)) room = { id: asText(tournament.room_id), password: asText(tournament.room_password) };
      }
    }
    ...
    const safeTournament = {
      ...
      game: tournament.games ? { id: tournament.games.id, name: asText(tournament.games.name), slug: asText(tournament.games.slug), shortName: asText(tournament.games.short_name), imageUrl: asText(tournament.games.image_url), bannerUrl: asText(tournament.games.banner_url) } : null,
    };

    const safeParticipants = (participants || []).map((p) => {
      const profile = profileMap.get(asText(p.user_id));
      return {
        id: asText(p.id),
        userId: asText(p.user_id),
        playerName: asText(p.player_name || profile?.full_name || profile?.username) || "Player",
    ...
    const safeResults = (results || []).map((result) => {
      const profile = profileMap.get(asText(result.user_id));
      return { ...result, userId: asText(result.user_id), playerName: asText(profile?.full_name || profile?.username) || "Player", avatarUrl: asText(profile?.avatar_url) };
    });

    return Response.json({ success: true, data: { tournament: safeTournament, participants: safeParticipants, results: safeResults, registration, room, registeredCount: safeParticipants.length, remainingSlots: Math.max(0, safeTournament.capacity - safeParticipants.length) }, error: null }, { headers: { "Cache-Control": "no-store" } });
  } catch (error) {
...
export async function POST(request, { params }) {
  try {
    ...
    const admin = adminClient();
    const { data: tournament, error: tournamentError } = await admin.from("tournaments").select("id,match_type").eq("id", Number(tournamentId)).maybeSingle();
    if (tournamentError) throw tournamentError;
    if (!tournament) return jsonError("Tournament not found.", 404, "TOURNAMENT_NOT_FOUND");
    ...
    for (const item of memberInputs) {
      ...
      if (identifier) {
        const { data: profiles, error: profileError } = await admin.from("profiles").select("id,username,email,full_name").or(`username.ilike.${identifier},email.ilike.${identifier}`).limit(2);
    ...
    const { data, error } = await client.rpc("register_tournament_entry", { p_tournament_id: Number(tournamentId), p_game_username: playerName, p_game_uid: gameUid, p_team_name: teamName, p_members: members });
    if (error) {
      const code = asText(error.message);
      const mapping = { AUTH_REQUIRED: [401,"AUTH_REQUIRED"], TOURNAMENT_NOT_FOUND: [404,"TOURNAMENT_NOT_FOUND"], ALREADY_REGISTERED: [409,"ALREADY_REGISTERED"], TEAM_MEMBER_ALREADY_REGISTERED: [409,"TEAM_MEMBER_ALREADY_REGISTERED"], TOURNAMENT_FULL: [409,"TOURNAMENT_FULL"], REGISTRATION_CLOSED: [409,"REGISTRATION_CLOSED"], REGISTRATION_NOT_OPEN: [409,"REGISTRATION_NOT_OPEN"], INSUFFICIENT_BALANCE: [402,"INSUFFICIENT_BALANCE"], PLAYER_DETAILS_REQUIRED: [422,"PLAYER_DETAILS_REQUIRED"], TEAM_NAME_REQUIRED: [422,"TEAM_NAME_REQUIRED"], TEAM_SIZE_INVALID: [422,"TEAM_SIZE_INVALID"], TEAM_MEMBER_INVALID: [422,"TEAM_MEMBER_INVALID"], TEAM_MEMBER_NOT_FOUND: [422,"TEAM_MEMBER_NOT_FOUND"], CAPTAIN_REQUIRED: [422,"CAPTAIN_REQUIRED"] };
      const [status, mappedCode] = mapping[code] || [400,"REGISTRATION_FAILED"];
      const messages = { INSUFFICIENT_BALANCE:"Your wallet balance is not enough for this entry fee.", ALREADY_REGISTERED:"You are already registered for this tournament.", TEAM_MEMBER_ALREADY_REGISTERED:"One of the selected team members is already registered.", TOURNAMENT_FULL:"This tournament is full.", REGISTRATION_CLOSED:"Registration is closed for this tournament.", REGISTRATION_NOT_OPEN:"Registration is not open yet." };
