...
export async function GET(request, { params }) {
  try {
    ...
    const { data: tournament, error: tournamentError } = await db
      .from("tournaments")
      .select("id,tournament_id,game_id,game_short_name,tournament_name,match_date,start_time,mode_format_id,map,entry_fee,per_kill,prize_pool,game_standards,gg_standard_statement,parts_of_day,publish,status,slots_of_mode,lock,active")
    ...
    if (gameByIdResult.error) throw gameByIdResult.error;
    ...
    if (!game && tournament.game_short_name) {
      const { data, error } = await db.from("games").select("id,game_name,game_short_name,image_url,status,publish,device,display_order").eq("game_short_name", tournament.game_short_name).maybeSingle();
    ...
    let results = [];
    const matchIds = matches.map((match) => text(match.match_id || match.id)).filter(Boolean);
    if (matchIds.length) {
    ...
    const safeRules = rules.map((rule) => ({
      title: text(rule.rule_title, text(rule.section, "RULE")),
      summary: text(rule.rule_description, "Tournament rule"),
      items: text(rule.rule_description, "").split(/\r?\n/).map((line) => line.replace(/^[-•]\s*/, "").trim()).filter(Boolean),
    }));
    const safePrizes = prizes.map((prize) => ({
      id: text(prize.prize_id || prize.id),
      position: number(prize.position, 0),
      type: text(prize.prize_type),
    ...
    const userIds = [...new Set([
      ...registrations.map((r) => text(r.user_id)),
      ...members.map((m) => text(m.user_id)),
    ].filter(Boolean))];
    let users = [];
    if (userIds.length) {
      const { data, error } = await db.from("users").select("user_id,player_id,full_name,avatar_url,email,username").in("user_id", userIds);
      if (error) throw error;
      users = data || [];
    }
    ...
    const safeMembers = members.map((member) => {
      ...
      return {
        id: text(member.id || member.team_member_id),
        userId: text(member.user_id),
        playerName: text(member.player_name || user?.full_name || user?.player_id, "Player"),
        uid: text(member.game_uid),
        team: text(team?.team_name),
        role: text(member.member_role),
        status: text(member.status, "joined"),
        joinedAt: member.joined_at || null,
        avatarUrl: text(user?.avatar_url),
        playerId: text(member.player_id || user?.player_id),
      };
    });
    ...
    const safeRegistrations = registrations.map((registration) => {
      ...
      return {
        ...
        registrationId: text(registration.registration_id),
        userId: text(registration.user_id),
        playerName: text(registration.player_name || user?.full_name || user?.player_id, "Player"),
        uid: text(registration.game_uid),
        team: text(team?.team_name),
        teamId: text(registration.team_id),
        type: text(registration.registration_type),
        fee: number(registration.registration_fee),
        paymentStatus: text(registration.payment_status),
        registrationStatus: text(registration.registration_status),
        registeredAt: registration.registered_at || null,
        avatarUrl: text(user?.avatar_url),
        playerId: text(registration.player_id || user?.player_id),
      };
    });
    ...
    if (user) {
      ...
      if (eligible) {
        ...
        const releasedRoom = rooms.find((candidate) => {
          ...
          return (text(candidate.status).toLowerCase() === "released") || (releaseAt && now >= releaseAt);
        });
        if (releasedRoom?.game_room_id && releasedRoom?.room_password) {
          room = {
            id: text(releasedRoom.game_room_id),
            password: text(releasedRoom.room_password),
            matchId: text(releasedRoom.match_id),
            releasedAt: releasedRoom.room_release_time || null,
          };
        }
      }
    ...
    const safeTournament = {
      ...
      matchType: text(formatResult.data?.team_size > 1 ? (formatResult.data?.team_size === 2 ? "duo" : "squad") : "solo", text(tournament.mode_format_id, "solo")),
      ...
      game: game
        ? { id: text(game.id), name: text(game.game_name, tournament.game_short_name || "Game"), shortName: text(game.game_short_name), imageUrl: text(game.image_url) }
        : { id: "", name: text(tournament.game_short_name, "Game"), shortName: text(tournament.game_short_name), imageUrl: "" },
      format: { id: text(formatResult.data?.format_id), name: text(formatResult.data?.display_name || formatResult.data?.format_name || tournament.mode_format_id), teamSize: number(formatResult.data?.team_size, 1) },
    };

    const safeMatches = matches.map((match) => ({
      ...match,
      id: text(match.id),
      matchId: text(match.match_id),
      name: text(match.match_name),
      date: match.match_date,
      time: match.match_time,
      map: text(match.map),
      status: text(match.status),
      resultStatus: text(match.result_status),
    }));
    ...
    return Response.json({
      ...
      data: {
        ...
        participants: safeMembers.length ? safeMembers : safeRegistrations,
        ...
        room,
        registeredCount,
        remainingSlots: capacity ? Math.max(0, capacity - registeredCount) : 0,
      },
      error: null,
...
export async function POST(request, { params }) {
  try {
    ...
    const { data: tournament, error: tournamentError } = await db
      .from("tournaments")
      .select("tournament_id,game_short_name,mode_format_id,match_date,start_time,entry_fee,status,slots_of_mode")
      .eq("tournament_id", tournamentId)
      .maybeSingle();
    if (tournamentError) throw tournamentError;
    ...
    const { data: game, error: gameError } = await db
      .from("games")
      .select("id,game_short_name,game_name")
      .eq("game_short_name", tournament.game_short_name)
      .maybeSingle();
    if (gameError) throw gameError;
    if (!game) return jsonError("This tournament is linked to an unavailable game.", 409, "GAME_NOT_FOUND");

    const { data: format, error: formatError } = await db.from("formats").select("format_id,game_id,game_short_name,team_size").eq("format_id", tournament.mode_format_id).maybeSingle();
    if (formatError) throw formatError;
    if (!format) return jsonError("This tournament format is not configured.", 409, "FORMAT_NOT_FOUND");
    if (format.game_short_name && text(format.game_short_name) !== text(game.game_short_name)) return jsonError("This tournament format belongs to a different game.", 409, "GAME_FORMAT_MISMATCH");
    ...
    const { data, error } = await authClient.rpc("register_tournament", {
    ...
    if (error) {
      ...
      const mapping = {
        ...
        TEAM_MEMBER_DUPLICATE: [422, "TEAM_MEMBER_DUPLICATE"],
        CAPACITY_NOT_CONFIGURED: [409, "CAPACITY_NOT_CONFIGURED"],
      };
      ...
      const messages = {
        ...
        GAME_UID_ALREADY_REGISTERED: "That Game UID is already registered for this tournament.",
