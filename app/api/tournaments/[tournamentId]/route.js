...
async function loadData(db, tournament) {
  const [gameById, format, rules, prizes, registrations, teams, members, matches, rooms] = await Promise.all([
    ...
    db.from("formats").select("format_id,format_name,display_name,category,team_size,game_short_name").eq("format_id", tournament.mode_format_id).maybeSingle(),
...
async function makePayload(request, db, tournament) {
  ...
  const registeredCount = safeMembers.filter((m) => !["cancelled", "rejected", "disqualified"].includes(m.status.toLowerCase())).length || safeRegistrations.length;
...
export async function GET(request, { params }) {
  try {
    ...
    return Response.json({ success: true, data: await makePayload(request, db, tournament), error: null }, { headers: { "Cache-Control": "no-store" } });
  } catch (error) {
...
export async function POST(request, { params }) {
  try {
    ...
    const format = await db.from("formats").select("format_id,format_name,display_name,category,team_size,game_short_name").eq("format_id", tournament.mode_format_id).maybeSingle();
